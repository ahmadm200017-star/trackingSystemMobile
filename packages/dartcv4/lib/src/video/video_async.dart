...
/// Async counterparts for [TrackerCSRT]. CSRT is the heaviest of the three
/// trackers, so running it on OpenCV's native thread pool rather than the Dart
/// isolate matters more here than it does for MIL.
extension TrackerCSRTAsync on TrackerCSRT {
  Future<void> initAsync(InputArray image, Rect boundingBox) async {
    cvAssert(boundingBox.x >= 0, "boundingBox.x must be >= 0");
    cvAssert(boundingBox.y >= 0, "boundingBox.y must be >= 0");
    cvAssert(
    ...
    return cvRunAsync0((callback) => cvideo.cv_TrackerCSRT_init(ref, image.ref, boundingBox.ref, callback), (
      c,
    ) {
      c.complete();
    });
  ...
  Future<(bool, Rect)> updateAsync(Mat img) async {
    final bBox = calloc<cvg.CvRect>();
    final p = calloc<ffi.Bool>();
    return cvRunAsync0((callback) => cvideo.cv_TrackerCSRT_update(ref, img.ref, bBox, p, callback), (c) {
      final rval = (p.value, Rect.fromPointer(bBox));
      calloc.free(p);
      return c.complete(rval);
    });
...
/// Async counterparts for [TrackerKCF].
extension TrackerKCFAsync on TrackerKCF {
  Future<void> initAsync(InputArray image, Rect boundingBox) async {
    cvAssert(boundingBox.x >= 0, "boundingBox.x must be >= 0");
    cvAssert(boundingBox.y >= 0, "boundingBox.y must be >= 0");
    cvAssert(
    ...
    return cvRunAsync0((callback) => cvideo.cv_TrackerKCF_init(ref, image.ref, boundingBox.ref, callback), (
      c,
    ) {
      c.complete();
    });
  ...
  Future<(bool, Rect)> updateAsync(Mat img) async {
    final bBox = calloc<cvg.CvRect>();
    final p = calloc<ffi.Bool>();
    return cvRunAsync0((callback) => cvideo.cv_TrackerKCF_update(ref, img.ref, bBox, p, callback), (c) {
      final rval = (p.value, Rect.fromPointer(bBox));
      calloc.free(p);
      return c.complete(rval);
    });
