...
///
/// Slower than [TrackerMIL] but markedly more robust: it estimates the target's
/// scale, so the returned box grows and shrinks with the object instead of
/// staying the size it was seeded with.
///
/// see: https://docs.opencv.org/4.x/d2/da2/classcv_1_1TrackerCSRT.html
class TrackerCSRT extends CvStruct<cvg.TrackerCSRT> {
  TrackerCSRT(cvg.TrackerCSRTPtr ptr, [bool attach = true]) : super.fromPointer(ptr) {
    if (attach) {
      finalizer.attach(this, ptr.cast(), detach: this);
    }
  }

  factory TrackerCSRT.create() {
    final p = calloc<cvg.TrackerCSRT>();
    cvRun(() => cvideo.cv_TrackerCSRT_create(p));
    return TrackerCSRT(p);
  }
  ...
  void init(InputArray image, Rect boundingBox) {
    ...
    cvRun(() => cvideo.cv_TrackerCSRT_init(ref, image.ref, boundingBox.ref, ffi.nullptr));
  }
  ...
  (bool, Rect) update(Mat img) {
    final bBox = calloc<cvg.CvRect>();
    final p = calloc<ffi.Bool>();
    cvRun(() => cvideo.cv_TrackerCSRT_update(ref, img.ref, bBox, p, ffi.nullptr));
    final rval = (p.value, Rect.fromPointer(bBox));
    calloc.free(p);
    return rval;
  }

  static final finalizer = OcvFinalizer<cvg.TrackerCSRTPtr>(cvideo.addresses.cv_TrackerCSRT_close);

  void dispose() {
    finalizer.detach(this);
    cvideo.cv_TrackerCSRT_close(ptr);
  }

  @override
  cvg.TrackerCSRT get ref => ptr.ref;
}

/// Kernelized Correlation Filter tracker.
///
/// Considerably faster than [TrackerCSRT] and a reasonable middle ground, though
/// it does not adapt to scale and recovers less well from occlusion.
///
/// see: https://docs.opencv.org/4.x/d2/dff/classcv_1_1TrackerKCF.html
class TrackerKCF extends CvStruct<cvg.TrackerKCF> {
  TrackerKCF(cvg.TrackerKCFPtr ptr, [bool attach = true]) : super.fromPointer(ptr) {
    if (attach) {
      finalizer.attach(this, ptr.cast(), detach: this);
    }
  }

  factory TrackerKCF.create() {
    final p = calloc<cvg.TrackerKCF>();
    cvRun(() => cvideo.cv_TrackerKCF_create(p));
    return TrackerKCF(p);
  }
  ...
  void init(InputArray image, Rect boundingBox) {
    ...
    cvRun(() => cvideo.cv_TrackerKCF_init(ref, image.ref, boundingBox.ref, ffi.nullptr));
  }
  ...
  (bool, Rect) update(Mat img) {
    final bBox = calloc<cvg.CvRect>();
    final p = calloc<ffi.Bool>();
    cvRun(() => cvideo.cv_TrackerKCF_update(ref, img.ref, bBox, p, ffi.nullptr));
    final rval = (p.value, Rect.fromPointer(bBox));
    calloc.free(p);
    return rval;
  }

  static final finalizer = OcvFinalizer<cvg.TrackerKCFPtr>(cvideo.addresses.cv_TrackerKCF_close);

  void dispose() {
    finalizer.detach(this);
    cvideo.cv_TrackerKCF_close(ptr);
  }

  @override
  cvg.TrackerKCF get ref => ptr.ref;
}
