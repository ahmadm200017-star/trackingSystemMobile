...
// TrackerCSRT and TrackerKCF live in the contrib tracking module, not in
// opencv2/video.hpp, so they need their own header and their own link entry.
...
CVD_TYPEDEF(cv::Ptr<cv::TrackerCSRT>, TrackerCSRT);
CVD_TYPEDEF(cv::Ptr<cv::TrackerKCF>, TrackerKCF);
...
CVD_TYPEDEF(void, TrackerCSRT);
CVD_TYPEDEF(void, TrackerKCF);
...
CvStatus* cv_TrackerCSRT_create(TrackerCSRT* rval);
CvStatus* cv_TrackerCSRT_init(TrackerCSRT self, Mat image, CvRect bbox, CvCallback_0 callback);
CvStatus* cv_TrackerCSRT_update(
    TrackerCSRT self, Mat image, CvRect* boundingBox, bool* rval, CvCallback_0 callback
);
void cv_TrackerCSRT_close(TrackerCSRTPtr self);

CvStatus* cv_TrackerKCF_create(TrackerKCF* rval);
CvStatus* cv_TrackerKCF_init(TrackerKCF self, Mat image, CvRect bbox, CvCallback_0 callback);
CvStatus* cv_TrackerKCF_update(
    TrackerKCF self, Mat image, CvRect* boundingBox, bool* rval, CvCallback_0 callback
);
void cv_TrackerKCF_close(TrackerKCFPtr self);
