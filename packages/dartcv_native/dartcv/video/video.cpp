...
CvStatus* cv_TrackerCSRT_create(TrackerCSRT* rval) {
    BEGIN_WRAP
    rval->ptr = new cv::Ptr<cv::TrackerCSRT>(cv::TrackerCSRT::create());
    END_WRAP
}
...
CvStatus* cv_TrackerKCF_update(
    ...
    BEGIN_WRAP
    // KCF reports loss through the return value only; the box is left untouched,
    // so it is cleared here rather than handing back an uninitialised rect.
    cv::Rect bb;
    *rval = (CVDEREF(self))->update(CVDEREF(image), bb);
    *boundingBox = {bb.x, bb.y, bb.width, bb.height};
    if (callback != nullptr) {
...
CvStatus* cv_TrackerKCF_create(TrackerKCF* rval) {
    BEGIN_WRAP
    rval->ptr = new cv::Ptr<cv::TrackerKCF>(cv::TrackerKCF::create());
    END_WRAP
}
